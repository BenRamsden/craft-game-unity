...
public class Inventory : MonoBehaviour {
    ...
    public bool addBlock(Block block){
		int itemBarIndex = checkForEntry(block.resourceString);
		if (itemBarIndex != -1) {
	...
	public string removeBlock(){
		...
		if (activeCount > 0) {
			string tempName = itemBar [activeList] [0].resourceString;
    ...
    public void setUI() {
		string materialName;
		for(int i = 0; i < 6; i++){
			materialName = (itemBar[i].Count > 0)? ((Block)(itemBar [i]) [0]).resourceString: "DefaultItem";
		...
		for(int i = 0; i < 6; ++i){
			if (itemBar [i].Count > 0) {
				if ((itemBar [i]) [0].GetType () == typeof(Block)) {
					if (((Block)(itemBar [i]) [0]).resourceString == typeString) {
